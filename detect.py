...
from train_data_augmented_tpu import normalize, l2_normalizer  
...
# Import constants  
from constants import (MODEL_WEIGHTS_PATH, ENCODINGS_PATH, REQUIRED_IMAGE_SIZE, CONFIDENCE_THRESHOLD, RECOGNITION_THRESHOLD, CLIPS_PATH, RECORDINGS_PATH)  
...
def generate_recognized_image(event_data, date_format):  
    ...
    source_img_path = os.path.join(CLIPS_PATH, f"GarageCamera-{event_id}-clean.png")  
    out_image_path = os.path.join(CLIPS_PATH, f"GarageCamera-{event_id}-rec.jpg")  
    ...
    video_path = os.path.join(RECORDINGS_PATH, date_format[:10], subfolder_num_str, 'GarageCamera', f"{date_format[14:16]}.{date_format[17:19]}.mp4")  
